...
class Runner(object):
    def __init__(self, params):
        ...
        self.time_stamp = time.strftime('%Y_%m_%d') + '_' + time.strftime('%H_%M_%S')
...
#返回的是一个dict类型的。
    ...
    def fit(self):
        ...
        #save_path = 'D:\\GithubRepo\\RGCN-LinkPrediction\\'+self.time_stamp + '.pt'
    ...
    def calc_mrr(self, output, split, hits, filtered=True):
        ranks = []
        rank_cal_limit=0 #The process to calculate the rank will consume too much time. setup the limit.
        for triple, label in iter(self.data_iter[split]):
            if rank_cal_limit<1000:
                ...
                rank_cal_limit=rank_cal_limit+1
...
#                         help="dataset to use")
# =============================================================================
    parser.add_argument("--dataset", type=str, default="DRKG",
                        help="dataset to use")
